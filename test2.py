...
res = printer_info1['result']
pprint.pprint(len(printer_info1['result'])) #инфа о моделях
#pprint.pprint(printer_info1['result'])
unsort_list = []
for i in range(len(res)-1):
    unsort_list.append((res[i]['path'][:-6],res[i]['modified'],time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(res[i]['modified']))))
